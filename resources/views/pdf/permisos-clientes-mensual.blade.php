...
<body>
    ...
    <div class="seccion">
        ...
        <table class="table">
            ...
            <tbody>
                <tr>
                    <td class="center">{{ (int) ($totales['pendientes'] ?? 0) }}</td>
                    <td class="center">{{ (int) ($totales['aprobados'] ?? 0) }}</td>
                    <td class="center">{{ (int) ($totales['rechazados'] ?? 0) }}</td>
                    <td class="center">{{ (int) ($totales['solicitados'] ?? 0) }}</td>
                </tr>
            </tbody>
        </table>
    </div>
    ...
    {{-- CLIENTES QUE ALCANZARON EL LÍMITE --}}
    <div class="seccion">
        ...
        <table class="table">
            ...
            <tbody>
                @forelse(($limiteClientes ?? []) as $lc)
                    <tr>
                        <td>{{ data_get($lc, 'cliente_nombre', '—') }}</td>
                        <td class="center">{{ (int) data_get($lc, 'c', 0) }}</td>
                    </tr>
                @empty
                    <tr>
        ...
        <table class="table">
            ...
            <tbody>
                @forelse(($detalle ?? collect()) as $p)
                    <tr>
                        <td>
                            {{ !empty($p->fecha) ? \Carbon\Carbon::parse($p->fecha)->format('d/m/Y') : '—' }}
                        </td>
                        <td>{{ optional($p->cliente)->nombre_completo ?? '—' }}</td>
                        <td>{{ isset($p->estado) ? ucfirst($p->estado) : '—' }}</td>
                        <td>{{ $p->motivo ?? '—' }}</td>
