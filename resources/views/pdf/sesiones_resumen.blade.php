...
<body>
    ...
    <div class="seccion">
        ...
        <div class="kpis">
            <div class="kpi"><div class="label">Sesiones</div><div class="val">{{ number_format($sesiones) }}</div></div>
            <div class="kpi"><div class="label">Clientes únicos</div><div class="val">{{ number_format($clientesUnicos) }}</div></div>
            <div class="kpi"><div class="label">Ingresos</div><div class="val">Bs {{ number_format($ingresos, 2) }}</div></div>
            <div class="kpi"><div class="label">Ticket promedio</div><div class="val">Bs {{ number_format($ticketPromedio, 2) }}</div></div>
        </div>
    </div>

    {{-- TOP tipos --}}
    <div class="seccion">
        <div class="titulo-seccion">Top 5 tipos de sesión</div>
        <table class="table">
            <thead>
                <tr>
                    <th>Tipo</th>
            ...
            <tbody>
                @forelse ($topTipos as $t)
                    @php $pct = $maxSesTipo ? round(($t->sesiones / $maxSesTipo) * 100) : 0; @endphp
                    <tr>
                        <td>{{ $t->tipo_sesion ?? '—' }}</td>
                        <td class="center">{{ $t->sesiones }}</td>
                        <td><div class="bar-wrap"><div class="bar" style="width: {{ $pct }}%;"></div></div></td>
                        <td class="right">Bs {{ number_format($t->ingresos, 2) }}</td>
                    </tr>
                @empty
    ...
    <div class="seccion">
        ...
        <table class="table">
            <thead>
                <tr><th>Día</th><th class="center">Sesiones</th><th class="right">Ingresos</th></tr>
            </thead>
            <tbody>
    ...
    <div class="seccion">
        ...
        <table class="table">
            <thead>
                <tr><th>Hora</th><th class="center">Sesiones</th><th class="right">Ingresos</th></tr>
            </thead>
            <tbody>
            ...
            <tbody>
                @forelse ($detalle as $s)
                    <tr>
                        ...
                        <td>{{ $s->cliente?->nombre }} {{ $s->cliente?->apellido_paterno }} {{ $s->cliente?->apellido_materno }}</td>
                        <td>{{ $s->instructor?->nombre }} {{ $s->instructor?->apellido_paterno }} {{ $s->instructor?->apellido_materno }}</td>
                        <td>{{ $s->tipo_sesion ?? '—' }}</td>
