...
<body>
    ...
    <div class="seccion">
        ...
        <div class="kpis">
            <div class="kpi">
                ...
                <div class="val">{{ number_format($ing_registros) }}</div>
            </div>
            <div class="kpi">
                <div class="label">Unidades</div>
                <div class="val">{{ number_format($ing_unidades) }}</div>
            </div>
            <div class="kpi">
                <div class="label">Paquetes</div>
                <div class="val">{{ number_format($ing_paquetes) }}</div>
            </div>
            <div class="kpi">
                <div class="label">Costo Total</div>
                <div class="val">Bs {{ number_format($ing_costo, 2) }}</div>
            </div>
        </div>
        ...
        <table class="table">
            ...
            <tbody>
                @forelse ($ingresos as $r)
                    <tr>
                        <td>{{ optional($r->producto)->nombre ?? '—' }}</td>
    ...
    <div class="seccion">
        ...
        @php $top = collect($topPorProducto ?? [])->take(5); @endphp
        <table class="table">
            <thead>
                <tr>
                    <th>Producto</th>
    ...
    <div class="seccion">
        ...
        <table class="table">
            ...
            <tbody>
                @forelse ($controlStock as $prod => $datos)
                    <tr>
                        <td>{{ $prod }}</td>
                        <td class="center">{{ (int) data_get($datos, 'inicial', 0) }}</td>
                        <td class="center">{{ (int) data_get($datos, 'ingresado', 0) }}</td>
                        <td class="center">{{ (int) data_get($datos, 'vendido', 0) }}</td>
                        <td class="center">{{ (int) data_get($datos, 'final', 0) }}</td>
                    </tr>
                @empty
                    <tr>
