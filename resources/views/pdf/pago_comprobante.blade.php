    ...
    <div class="brand-band">
        ...
        <div class="brand-body">
            ...
            <div class="meta">
                ...
                <div><strong>Fecha:</strong> {{ $generado_el }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <table>
            <tbody>
                <tr class="row">
                    <th>Personal</th>
                    <td>{{ $pago->personal->nombre_completo ?? '—' }}</td>
                </tr>
                <tr class="row">
                ...
                <tr class="row">
                    <th>Método</th>
                    <td>
                        @php $m = strtolower($pago->metodo_pago ?? ''); @endphp
                        @if($m === 'qr')
                            <span class="badge badge-pay">QR</span>
                        @elseif($m === 'efectivo')
                            <span class="badge badge-cash">Efectivo</span>
                        @else
                            <span class="badge">{{ strtoupper($pago->metodo_pago ?? '—') }}</span>
                        @endif
                    </td>
                </tr>
                <tr class="row">
                    <th>Monto (Bs)</th>
                    <td class="right"><strong>{{ number_format($pago->monto, 2) }}</strong></td>
                </tr>
                <tr class="row">
                    <th>Estado</th>
                    <td>
                ...
                <tr class="row">
                    <th>Turno</th>
                    <td>{{ $pago->turno->nombre ?? '—' }}{{ $pago->turno?->dia_nombre ? ' ('.$pago->turno->dia_nombre.')' : '' }}</td>
                </tr>
                <tr class="row">
                    <th>Sala</th>
